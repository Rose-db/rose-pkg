...
var _ = GinkgoDescribe("Read tests", func() {
	...
	GinkgoIt("Should assert that pagination returns the correct documents with readBy with float index", func() {
		a := testCreateRose(false)

		collName := testCreateCollection(a, "test_coll")

		err := a.NewIndex(collName, "price", floatIndexType)
	...
	GinkgoIt("Should assert that pagination returns the correct documents with readBy with bool index", func() {
		a := testCreateRose(false)

		collName := testCreateCollection(a, "test_coll")

		err := a.NewIndex(collName, "isValid", boolIndexType)
