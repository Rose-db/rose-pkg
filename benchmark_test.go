...
func BenchmarkAppReadTenThousand(b *testing.B) {
	benchmarkRemoveFileSystemDb()
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			panic(fmt.Sprintf("Could not write with message: %s", err.Error()))

			return
		}

		if res.Status != OkResultStatus {
			panic(fmt.Sprintf("Invalid write status given: %s, id: %s", res.Status, id))
		}
	}
	...
	if err != nil {
		panic(fmt.Sprintf("Cannot not shutdown with error: %s", err.Error()))
	}
	...
	for n := 0; n < b.N; n++ {
		for i := 0; i < n; i++ {
			...
			if err != nil {
				panic(fmt.Sprintf("Could not read with message: %s", err.Error()))

				return
			}

			if res.Status != FoundResultStatus {
				panic(fmt.Sprintf("Benchmark failed. Invalid status: %s", res.Status))
			}
		}
	}
...
func BenchmarkAppReadHundredThousand(b *testing.B) {
	...
	if err := a.Shutdown(); err != nil {
		panic(fmt.Sprintf("Error in benchmark: %s", err.Error()))
	}
	...
	if err != nil {
		panic(fmt.Sprintf("Error shuting down: %s", err.Error()))
	}
}
